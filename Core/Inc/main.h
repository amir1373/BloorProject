/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2024 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim);

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define Emergency_Pin GPIO_PIN_3
#define Emergency_GPIO_Port GPIOE
#define Input2_Pin GPIO_PIN_4
#define Input2_GPIO_Port GPIOE
#define Relay2_Pin GPIO_PIN_6
#define Relay2_GPIO_Port GPIOE
#define MotorCurrentSense_Pin GPIO_PIN_1
#define MotorCurrentSense_GPIO_Port GPIOA
#define MOTOR_EN1_Pin GPIO_PIN_6
#define MOTOR_EN1_GPIO_Port GPIOA
#define MOTOR_EN2_Pin GPIO_PIN_7
#define MOTOR_EN2_GPIO_Port GPIOA
#define LED_ACK_Pin GPIO_PIN_7
#define LED_ACK_GPIO_Port GPIOE
#define IR_SENSOR2_Pin GPIO_PIN_12
#define IR_SENSOR2_GPIO_Port GPIOB
#define Patient_key_Pin GPIO_PIN_13
#define Patient_key_GPIO_Port GPIOB
#define IR_Sensor_Pin GPIO_PIN_14
#define IR_Sensor_GPIO_Port GPIOB
#define IR_Sensor_EXTI_IRQn EXTI15_10_IRQn
#define ENCODER_BMW_Pin GPIO_PIN_13
#define ENCODER_BMW_GPIO_Port GPIOD
#define ENCODER_BMW_EXTI_IRQn EXTI15_10_IRQn
#define Buzzer_Pin GPIO_PIN_15
#define Buzzer_GPIO_Port GPIOD
#define USART1_DIR_Pin GPIO_PIN_5
#define USART1_DIR_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
